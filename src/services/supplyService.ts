...
export class SupplyService {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('check constraint')) {
          return { 
            success: false, 
            error: 'Valores de estoque devem ser não negativos e nível mínimo deve ser menor ou igual ao máximo.' 
          };
        }
  ...
  static async updateSupply(
    ...
    try {
      const updatePayload: Partial<SupplyInsert> = {};
      
      if (updateData.name !== undefined) updatePayload.name = updateData.name.trim();
      if (updateData.unit !== undefined) updatePayload.unit = updateData.unit.trim();
